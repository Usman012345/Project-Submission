...
root = tk.Tk()
root.title("Gesture Recognition UI")

# Add a label for the mode
mode_label = tk.Label(root, text="Mode: Gesture Recognition", font=("Helvetica", 14))
mode_label.pack(pady=10)

# Add a scrolled text box for displaying recognized text
text_area = scrolledtext.ScrolledText(root, wrap=tk.WORD, width=50, height=10, font=("Helvetica", 12))
...
clear_button = tk.Button(root, text="Clear", command=clear_text, font=("Helvetica", 12))
clear_button.pack(pady=5)

# Function to update the text box from the recognition program
def update_text_area(new_text):
    text_area.insert(tk.END, f"{new_text}\n")
    text_area.see(tk.END)  # Automatically scroll to the end of the text

# Update the `say_text` function to include text updates in the UI
def say_text(text):
    global is_voice_on
    if not is_voice_on:
        return
...
# Run the gesture recognition in a separate thread to keep the UI responsive
def run_recognition():
    recognize()

recognition_thread = Thread(target=run_recognition, daemon=True)
